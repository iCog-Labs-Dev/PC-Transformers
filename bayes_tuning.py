...
def objective(trial):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"  Combined Energy: {combined_energy:.4f}")
            ...
            with open(trial_log_path, "a") as f:
                ...
                f.write(f"CE Loss: {val_loss:.6f} | Raw Energy: {avg_energy:.6f} | Norm Energy: {normalized_energy:.6f} | Combined: {combined_energy:.6f}\n")
...
def run_tuning(n_trials=30, study_name="bayesian_tuning"):
    ...
    with open(summary_log_path, "w") as f:
        ...
        f.write(f"Objective: Minimize combined energy (normalized internal energy + CE loss)\n\n")
        f.write(f"Trial Progress:\n")
        f.write(f"{'Trial':<6} {'Time(s)':<8} {'CE Loss':<10} {'Raw Energy':<12} {'Norm Energy':<12} {'Combined':<12} {'Energy Fn':<12}\n")
        f.write(f"{'-'*82}\n")
    ...
    def log_trial_callback(study, trial):
        ...
        if trial.state == optuna.trial.TrialState.COMPLETE:
            with open(summary_log_path, "a") as f:
                ...
                combined_energy = trial.user_attrs.get("combined_energy", float('inf'))
                ...
                f.write(f"{trial.number:<6} {trial_time:<8.1f} {ce_loss:<10.4f} {energy:<12.6f} {normalized_energy:<12.6f} {combined_energy:<12.6f} {energy_fn:<12}\n")
    try:
        ...
        if study.best_trial:
            ...
            with open(results_path, "w") as f:
                f.write(f"COMBINED ENERGY OPTIMIZATION RESULTS\n")
                f.write(f"====================================\n\n")
                ...
                f.write(f"  Combined Energy: {trial.user_attrs.get('combined_energy', 'N/A'):.4f}\n\n")
