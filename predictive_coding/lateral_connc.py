...
class LateralConnections(nn.Module):
    ...
    def update_weights(self, x: torch.Tensor, optimizer=None, clip_value: float = None):
        """Anti-Hebbian weight update for decorrelation. """
        with torch.no_grad():
            anti_hebbian = -torch.einsum("bsh,bsv->hv", x, x)
            if optimizer is not None:
                optimizer.step_param(self.W_lateral, anti_hebbian, self.local_lr, clip_value=clip_value)
            else:
