...
class PCTransformer(nn.Module):
    ...
    def forward(self, target_ids, input_ids, use_kv_cache=False):
        ...
        embed_T = max(1, int(getattr(self.config, "embed_T", 1)))
        attn_T = max(1, int(getattr(self.config, "attn_T", 1)))
        linear_attn_T = max(1, int(getattr(self.config, "linear_attn_T", 1)))
        fc1_T = max(1, int(getattr(self.config, "fc1_T", 1)))
        fc2_T = max(1, int(getattr(self.config, "fc2_T", 1)))
        linear_output_T = max(1, int(getattr(self.config, "linear_output_T", 1)))
        max_T = max(embed_T, attn_T, linear_attn_T, fc1_T, fc2_T, linear_output_T)
        ...
        for t in range(max_T):
            ...
            for idx in range(len(self.blocks) - 1, -1, -1):
                ...
                if use_kv_cache and t == attn_T - 1:
