    ...
    def forward(self, target_ids, input_ids, use_kv_cache=False):
        ...
        for idx in range(len(self.blocks) - 1, -1, -1):
            ...
            for t_attn in range(getattr(self.config, 'attn_T', 1)):
                ...
                if idx == 0:
                    td_embed = self.embedding.pc_layer.get_td_err("embed") if t_attn > 0 else None
                else:
                    td_embed = self.blocks[idx - 1].mlp.pc_layer2.get_td_err("fc2") if t_attn > 0 else None
                if td_embed is None:
                    _debug_none_td("linear_attn.td_embed", "linear_attn", t_attn, idx)
