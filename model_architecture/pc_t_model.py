...
class PCTransformer(nn.Module):
    ...
    def forward(self, target_ids, input_ids, use_kv_cache=False):
        ...
        # Build projection latents q_* first, then copy q -> x
        with torch.no_grad():
        ...
        # Run a single q update pass using the same targets as x, then copy q -> x
        with torch.no_grad():
            q_current = self.embedding.pc_layer.init_q(
                batch_size=B,
                seq_len=S,
        ...
        # Refresh x from the updated q caches
        self.embedding.pc_layer.init_x(
            batch_size=B,
            seq_len=S,
            layer_type="embed",
        ...
        # print(f"[InitDebug] call={self._debug_init_print_count}")
        # print(
