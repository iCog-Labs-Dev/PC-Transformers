...
class PCTransformer(nn.Module):
    ...
    def forward(self, target_ids, input_ids):
        ...
        dummy_target = torch.zeros(batch_size, seq_len, H, device=device)


        self.embedding.pc_layer.forward(
            target_activity=dummy_target,
        ...
        for block in self.blocks:
            block.attn.pc_qkv.forward(
                target_activity=dummy_target,
                proj_layers={"q_proj": block.attn.query, "k_proj": block.attn.key, "v_proj": block.attn.value},
                layer_type="attn"
            )
            ...
            block.mlp.pc_layer1.forward(
                target_activity=torch.zeros(batch_size, seq_len, 4 * H, device=device),
                layer=block.mlp.fc1,
                layer_type="fc1"
            )
        ...
        self.output.pc_layer.forward(
            target_activity=torch.zeros(batch_size, seq_len, V, device=device),
        ...
        for t in range(self.config.T):
            ...
            for block_idx, block in enumerate(self.blocks):
                ...
                target = (self.blocks[block_idx + 1].attn.pc_qkv.get_x("attn")
        ...
        # Compute logits
        logits = self.output.pc_layer.get_x("linear") @ self.output.output.weight.T + self.output.output.bias
        return logits
